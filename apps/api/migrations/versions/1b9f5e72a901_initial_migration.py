...
def upgrade() -> None:
    ...
    # Для SQLite используем обычный String вместо UUID и задаем значения Enum напрямую
    ...
    # Создание enum типов для SQLite
    if is_sqlite:
        # SQLite не поддерживает типы Enum, поэтому создадим строки с ограничениями
        ...
        # Создаем CHECK ограничения для эмуляции Enum в SQLite
        ...
        # PostgreSQL поддерживает Enum, используем их
        ...
        # Для PostgreSQL не нужны дополнительные ограничения
    ...
    # Добавляем CHECK ограничения для SQLite
    ...
    # Создаем SQL для представления DebtBalances (только для PostgreSQL)
    if not is_sqlite:
        op.execute("""
        ...
        # Для SQLite создаем простой VIEW без специфичных для PostgreSQL функций
        op.execute("""
...
def downgrade() -> None:
    ...
    # Удаляем типы Enum (только для PostgreSQL)
    if not is_sqlite:
